...
* Pivotics stores the state of the UI in the URL so you can easly send links to other people.
...
Download <a href="https://github.com/os69/pivotics/archive/master.zip">Pivotics</a> and unzip the archive. Switch to pivotics
pivotics directory and start the included python based webserver by
...
## Type
Currently the following types are supported: String, Integer, Float and Icon.
## Initial Value
...
then cell fields are now listed sequentially on rows.
...
## Subtotals
Enter a number into the Number of sum fields field to get subtotal. The determines on how many levels you will get the subtotal.
...
you could end with a lot of empty columns (in the dev area).
